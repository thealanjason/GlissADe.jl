...
@testset "Adaptive Timestep (computeTimeStep) AD & Float64 test" begin
    ...
    sol_obj = Solution(
        ...
        Cells = Cells, location = "./test_dt_tmp1", points = points, faces = faces,
    )
    solver = Solver(sol_obj)
    caches = [GlissADe.Cache{Float64, GlissADe.INT_TYPE[], Float64}()]
    dt_f64 = GlissADe.computeTimeStep(solver, 0.5, GlissADe._mean_delta(Cells), caches) * 0.4
    ...
    so = Solution(
        ...
        p_MAX_RESIDUAL = 1e-4, h_MAX_RESIDUAL = 5e-1, u_MAX_RESIDUAL = 5e-1,
        MAX_ITERS = 60, MIN_ITERS = 50, h_clip = 0.0, h_min = 1e-3,
        Cells = Cs, location = "./test_dt_tmp2", points = pts, faces = fs,
    )
    slv = Solver(so)
