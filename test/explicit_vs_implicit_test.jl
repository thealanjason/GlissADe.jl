...
@testset "Explicit vs Golden JLD2 Reference Comparison" begin
    ...
    for method in (:euler, :rk2, :ssprk3, :rk4, :rk45)
        @testset "compare :$method vs golden JLD2" begin
            ...
            vol_exp_final =
                sum(sol_exp[end][5*i-4] * Cells[i].area for i in eachindex(Cells))
            ...
            rel_diff_h =
                norm(h_exp[wet_cells] - h_gold[wet_cells]) / norm(h_gold[wet_cells])
            rel_diff_u =
                norm(u_exp[wet_cells] - u_gold[wet_cells]) /
                (norm(u_gold[wet_cells]) + 1e-3)
