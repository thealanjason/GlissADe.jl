...
- Generic typing throughout &rarr; `Dual` numbers flow through the whole solve, no rewrite.
- Linear solve is the exception: differentiate $Au=b$ directly, $A\,du/dx = db/dx - (dA/dx)\,u$, reusing the primal factorization [@martinsning2021].
- Upwind/central switching, in-place updates (`.=`, `+=`): all just work, no special-casing.
- Every gradient validated against finite differences in the test suite.
