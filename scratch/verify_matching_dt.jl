...
function test_matching_dt()
    ...
    sol_obj_imp = Solution(
        ...
        alpha_p = 1.0,
        alpha_u = 1.0,
        alpha_h = 1.0, # Un-relaxed for transient accuracy
        Cells = Cells,
        location = "./tmp_imp_small_dt",
        points = points,
        faces = faces,
    )
    ...
    u_mag_imp = [
        sqrt(sol_imp[end][5*i-3]^2 + sol_imp[end][5*i-2]^2 + sol_imp[end][5*i-1]^2) for
    ...
    @printf(
        "  Explicit RK2 Steps: %d, Max Vel: %.4f m/s\n",
        length(t_exp),
        maximum(u_mag_exp)
    )
    @printf(
        "  Relative Difference in Thickness (h): %.4e (%.2f%%)\n",
        rel_diff_h,
        rel_diff_h * 100
    )
    @printf(
        "  Relative Difference in Velocity (u):  %.4e (%.2f%%)\n",
        rel_diff_u,
        rel_diff_u * 100
    )
    println("=========================================================================")
