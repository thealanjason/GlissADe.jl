...
function benchmark_rk45()
    ...
    init(threads = false, stats = true, plots = false, implicit = false, explicit_method = :rk45)
    ...
    sol_obj_exp = Solution(
        ...
        Cells = Cells, location = "./tmp_exp_rk45_bench", points = points, faces = faces, explicit_method = :rk45,
    )
    ...
    u_max_final = maximum(sqrt(sol_exp[end][5*i-3]^2 + sol_exp[end][5*i-2]^2 + sol_exp[end][5*i-1]^2) for i in eachindex(Cells))

    println("=========================================================================")
    @printf("RK45 Completed in %.3f seconds (%d steps) | Max Vel: %.4f m/s\n", t_elapsed, length(t_exp), u_max_final)
    println("=========================================================================")
end
