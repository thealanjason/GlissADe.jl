...
function analyze_physics()
    ...
    @printf("Bed Slope Angle Range: %.2f° to %.2f° (Average: %.2f°)\n", minimum(angles_deg), maximum(angles_deg), sum(angles_deg)/length(angles_deg))
    @printf("Tangential Gravity Acceleration (g*sin θ): %.2f m/s² to %.2f m/s²\n", minimum(g_parallel), maximum(g_parallel))
    @printf("Basal Friction Deceleration (μ*g*cos θ):  %.2f m/s² to %.2f m/s² (assuming μ=%.2f)\n", minimum(a_friction), maximum(a_friction), mu_val)
    @printf("Net Downhill Acceleration Rate (a_net):    %.2f m/s² to %.2f m/s²\n", minimum(a_net), maximum(a_net))
    @printf("\nTheoretical Rigid-Block Speed after t = 1.0s: v = a_net * 1.0s = %.2f m/s to %.2f m/s\n", minimum(v_theoretical_1s), maximum(v_theoretical_1s))
    println("=========================================================================")
end
