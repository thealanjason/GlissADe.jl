...
@inline function _local_coords(n, p1, p2; axis = 2)
    if (axis == 2)
        @inbounds localY = _normalize(p2 - p1)
        localX = _ncross(localY, n)
        return [localX, localY, n]
    elseif (axis == 1)
        @inbounds localX = _normalize(p2 - p1)
        localY = _ncross(n, localX)
        return [localX, localY, n]
    end
end
...
function _areas(points, faces, centers)
    ...
    @inbounds @maybe_threads Threads.nthreads() == 1 || !THREADS[] for i in eachindex(faces)
        ...
        @inbounds for j in eachindex(faces[i])
            ...
            area = area + _mag(_cross(internal_edge1, internal_edge2)) * 0.5
        end
        areas[i] = area
    end
    return areas
...
function _bi_transforms(centers, edge_centers, normals, points, faces, neighbours)
    ...
    @inbounds @maybe_threads Threads.nthreads() == 1 || !THREADS[] for i in eachindex(faces)
        ...
        @inbounds for j in eachindex(edges_i)
            ...
            Pe = _mag2(center, edge_centers_i[j]) # Allocates
            Pen = _mag2(centers[n], edge_centers_i[j]) + Pe # Allocates
            frac = one(T) - Pe / Pen
            normal_edge = _normalize(frac .* normal_i + (1 - frac) .* normals[n]) # Allocates
            ...
            transform_edge = _DCM(local_edge)
