...
function updateMomentum!(
    ...
    @inbounds @maybe_threads Threads.nthreads() == 1 || !threads for i in eachindex(Cells)
        ...
        if (t < 2.0 * dt)
            ...
            @inbounds Bv[3*i-2] =
                dt_inv * prev_solution[5*i-4] * prev_solution[5*i-3] * area
            @inbounds Bv[3*i-1] =
                dt_inv * prev_solution[5*i-4] * prev_solution[5*i-2] * area
            ...
            @inbounds Bv[3*i-2] =
                ...
                area *
                (
                    2.0 * prev_solution[5*i-4] * prev_solution[5*i-3] -
                    0.5 * pprev_solution[5*i-4] * pprev_solution[5*i-3]
                )
            @inbounds Bv[3*i-1] =
                dt_inv *
                area *
                (
                    2.0 * prev_solution[5*i-4] * prev_solution[5*i-2] -
                    0.5 * pprev_solution[5*i-4] * pprev_solution[5*i-2]
                )
            @inbounds Bv[3*i] =
                dt_inv *
                area *
                (
                    2.0 * prev_solution[5*i-4] * prev_solution[5*i-1] -
                    0.5 * pprev_solution[5*i-4] * pprev_solution[5*i-1]
                )
        end
        ...
        for i1 = (3*i-2):(3*i)
            @inbounds Av[i1, i1] +=
                rho_inv *
                area *
                solver.basal_stress(Cell, h0[i], vel_i, p[i], alpha, zeta, rho)
        end
    ...
    @inbounds @maybe_threads Threads.nthreads() == 1 || !threads for i in eachindex(Cells)
        if (
            Av[3*i-2, 3*i-2] < 1.0e-8 * one(W) ||
            Av[3*i-1, 3*i-1] < 1.0e-8 * one(W) ||
            Av[3*i-2, 3*i-2] < 1.0e-8 * one(W) ||
            h0[i] <= solver.h_min
        )
            for i1 = (3*i-2):(3*i)
