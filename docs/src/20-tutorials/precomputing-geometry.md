...
The third argument is the datatype to use for the state variables. Pass `Float64` for a regular solve, or `eltype(x)` when differentiating with respect to a parameter vector `x` (see [Differentiating a Simulation](differentiating-a-simulation.md)).
